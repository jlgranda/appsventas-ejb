 ...
 */
@Stateless
public class EmployeeService extends BussinesEntityHome<Employee> {
    ...
    public List<Employee> find(int maxresults, int firstresult) {

        CriteriaBuilder builder = getCriteriaBuilder();
        CriteriaQuery<Employee> query = builder.createQuery(Employee.class);

        Root<Employee> from = query.from(Employee.class);
        query.select(from).orderBy(builder.desc(from.get(Employee_.name)));
