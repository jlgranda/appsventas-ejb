...
//    @NamedQuery(name = "Product.findTopProductIdsBetweenOrg", query = "SELECT p.id,  sum(d.amount) FROM Detail d JOIN d.product p WHERE not p.id in (75, 676,672) and p.organization=?1 and d.createdOn >= ?2 and d.createdOn <= ?3 GROUP BY p ORDER BY 2 DESC"),
...
//    @NamedQuery(name = "Product.findTopProductIdsBetweenPriceOrg", query = "SELECT p.id,  sum(d.price*d.amount) FROM Detail d JOIN d.product p WHERE not p.id in (75, 676,672) and p.organization=?1 and d.createdOn >= ?2 and d.createdOn <= ?3 GROUP BY p ORDER BY 2 DESC"),
...
//    @NamedQuery(name = "Product.findTopProductIdsBetweenCategoryOrg", query = "SELECT p.category.name,  sum(d.price*d.amount) FROM Detail d JOIN d.product p WHERE not p.id in (75, 676,672) and p.organization=?1 and d.createdOn >= ?2 and d.createdOn <= ?3 GROUP BY p.category.name ORDER BY 2 DESC"),
