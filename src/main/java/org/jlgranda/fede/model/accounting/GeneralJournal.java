...
@NamedQueries({
    ...
    @NamedQuery(name = "GeneralJournal.findByCreatedOnAndOrg", query = "select s FROM GeneralJournal s WHERE s.emissionDate >= ?1 and s.emissionDate<= ?2 and s.organization = ?3 AND s.deleted = false ORDER BY s.id DESC"),
    @NamedQuery(name = "GeneralJournal.findByCreatedOnAndOrganization", query = "SELECT s FROM GeneralJournal s WHERE s.emissionDate >= ?1 AND s.emissionDate <= ?2 AND s.organization = ?3 AND s.deleted = false ORDER BY 1"),
