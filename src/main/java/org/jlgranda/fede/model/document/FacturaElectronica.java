...
@NamedQueries({
    ...
    @NamedQuery(name = "FacturaElectronica.findTopTotalProviderIdsBetween", query = "select sb.firstname, sb.initials, sum(f.importeTotal) from FacturaElectronica f JOIN f.subject sb WHERE f.author=sb.id, and f.fechaEmision >= ?1 and f.createdOn <= ?2 GROUP BY sb ORDER BY 2 DESC")
})
@XmlRootElement
