...
@NamedQueries({
    ...
    @NamedQuery(name = "FacturaElectronica.findTopTotalProviderIdsBetween", query = "select sb.initials, sum(f.importeTotal), sb.firstname, sb.surname from FacturaElectronica f JOIN f.author sb WHERE f.author=sb.id and f.fechaEmision >= ?1 and f.createdOn <= ?2 GROUP BY sb ORDER BY sum(f.importeTotal) DESC"),
})
@XmlRootElement
