...
@NamedQueries({
    ...
    @NamedQuery(name = "FacturaElectronica.findByOwnerAndEmisionAndEmissionType", query = "SELECT f FROM FacturaElectronica f WHERE f.owner = ?1 and f.fechaEmision  >= ?2 and f.fechaEmision <= ?3 and m.emissionType=?4 and f.active=?5 ORDER BY f.fechaEmision DESC"),
